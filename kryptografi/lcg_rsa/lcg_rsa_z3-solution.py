...
p = BitVec('p', pbits)
q = p*a + b
# p = Concat(BitVecVal(0, pbits), p)
# q = Concat(BitVecVal(0, pbits), q)
...
while True:
    ...
    if status != sat:
        ...
        exit()
    m = S.model()
    p_val, q_val = m.eval(p).as_long(), m.eval(q).as_long()
    print(f'Found one possible solution:')
    print(p_val)
    print(q_val)
    if p_val*q_val == n:
        print(f'Found true solution')
    S.add(p != p_val)
exit()
